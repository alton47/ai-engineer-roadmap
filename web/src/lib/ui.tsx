    ...
    <div className="px-8 pt-8 pb-6 border-b border-line">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          {sub && (
            <p className="font-mono text-[11px] text-faint mt-1.5 leading-relaxed">
              {sub}
            </p>
          )}
        </div>
...
export function Card({
  ...
  return (
    <div
      className={clsx(
        "bg-bg1 border border-line rounded-xl relative overflow-hidden shadow-card hover:border-line2 transition-colors duration-150 noise",
        p,
        className,
      )}
    >
...
export function CardLabel({
  ...
  return (
    <div className="flex items-center justify-between gap-3 mb-4">
      <span className="font-mono text-[10px] tracking-[.12em] uppercase text-faint font-medium">
        {children}
      </span>
      {right && (
        <div className="flex-shrink-0 text-[10px] font-mono text-faint">
          {right}
        </div>
      )}
    </div>
...
export function BarRow({
  ...
  return (
    <div className="flex items-center gap-3 group cursor-default">
      <div
        className="font-mono text-[11px] text-muted text-right w-28 flex-shrink-0 truncate group-hover:text-tx transition-colors"
        title={name}
      >
        {name}
      </div>
...
export function FL({ children }: { children: React.ReactNode }) {
  return (
    <span className="font-mono text-[10px] text-faint tracking-wide flex-shrink-0 select-none">
      {children}
    </span>
  );
}
export function FB({
  ...
  return (
    <button
      ...
      style={
        active ? { background: accent ?? "#b8ff3c", color: "#030303" } : {}
      }
    >
      {children}
    </button>
...
export function FSep() {
  return <div className="w-px h-3.5 bg-line flex-shrink-0 mx-1" />;
}
...
export function Chip({
  ...
  return (
    <span
      className={clsx(
        "font-mono text-[10px] px-2 py-0.5 rounded-md border inline-block",
        BS[color],
      )}
    >
      {children}
...
export function LevelChip({ v }: { v: string }) {
  ...
  return (
    <Chip color={m[(v ?? "").toLowerCase()] ?? "default"}>{v || "—"}</Chip>
  );
}
...
export function TT({
  ...
  return (
    <div className="bg-bg2 border border-line2 rounded-xl px-4 py-3 font-mono text-[11px] shadow-2xl min-w-36 max-w-52">
      {label && (
        <div className="text-hi font-semibold mb-2 text-[12px] font-sans">
          {label}
        </div>
      )}
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2 py-[3px]">
          <div
            className="w-2 h-2 rounded-full flex-shrink-0 glow-dot"
            style={{ background: p.color, color: p.color }}
          />
          <span className="text-muted truncate">{p.name}:</span>
          <span className="text-hi ml-auto pl-2">
            {fmt ? fmt(p.value, p.name) : p.value.toLocaleString()}
          </span>
        </div>
      ))}
    </div>
...
export function Spin({ size = 28 }: { size?: number }) {
  return (
    <div
      className="rounded-full border-2 border-line2 border-t-acid animate-spin"
...
export function DemoBanner() {
  return (
    ...
    >
      <span className="font-mono text-[10px] text-gold uppercase tracking-widest flex-shrink-0">
        ⚠ Demo data
      </span>
