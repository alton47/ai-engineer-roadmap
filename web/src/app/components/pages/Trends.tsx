...
export default function Trends({ data }: { data: AggregatedData }) {
  ...
  const topSkillNames = useMemo(() => {
    ...
    trend.forEach((pt) =>
      pt.top_skills.forEach((s) => {
        f[s.name] = (f[s.name] ?? 0) + s.count;
      }),
    );
  ...
  const skillRows = useMemo(
    () =>
      trend.map((pt) => {
        ...
        topSkillNames.forEach((sk) => {
          r[sk] = pt.top_skills.find((s) => s.name === sk)?.count ?? 0;
        });
        return r;
      }),
    [trend, topSkillNames],
  ...
  const topRoleNames = useMemo(() => {
    const f: Record<string, number> = {};
    trend.forEach((pt) =>
      pt.role_types.forEach((r) => {
        f[r.name] = (f[r.name] ?? 0) + r.count;
      }),
    );
    return Object.entries(f)
      .sort((a, b) => b[1] - a[1])
  ...
  const roleRows = useMemo(
    () =>
      trend.map((pt) => {
        const r: Record<string, string | number> = { month: pt.month };
        topRoleNames.forEach((rn) => {
          r[rn] = pt.role_types.find((x) => x.name === rn)?.count ?? 0;
        });
        return r;
      }),
    [trend, topRoleNames],
  ...
  const rising = useMemo(() => {
    ...
    const first = Object.fromEntries(
      trend[0].top_skills.map((s) => [s.name, s.count]),
    );
    const last = Object.fromEntries(
      trend[trend.length - 1].top_skills.map((s) => [s.name, s.count]),
    );
    return Object.keys(last)
      .map((name) => ({
        ...
        prev: first[name] ?? 0,
        growth: first[name]
          ? Math.round(((last[name] - first[name]) / first[name]) * 100)
          : 999,
      }))
  ...
  return (
    <PageWrap>
      <PageHeader
        ...
        sub={
          isDemoTrend
            ? "Showing historical demo data — run scraper monthly to populate real trends"
            : `${trend.length} data points · ${trend[0]?.month} → ${trend[trend.length - 1]?.month}`
        }
        badge={
          isDemoTrend ? (
            <span className="font-mono text-[10px] px-2.5 py-1 rounded-full border border-gold/30 text-gold bg-gold/5">
              demo trend
            </span>
          ) : undefined
        }
      ...
      <Section>
        <Filters>
          <FL>View:</FL>
          {VIEWS.map((v) => (
            <FB key={v.id} active={view === v.id} onClick={() => setView(v.id)}>
              {v.label}
            </FB>
          ))}
        </Filters>
        ...
        <Card className="mb-5" p="p-5">
          <CardLabel right={view === "salary" ? "USD / year" : "job count"}>
            ...
            {view === "skills" &&
              `Top ${topSkillNames.length} Skills — Demand Growth`}
              ...
              >
                <defs>
                  <linearGradient id="gVol" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#b8ff3c" stopOpacity={0.15} />
              ...
              >
                <defs>
                  <linearGradient id="gSal" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#3cf0ff" stopOpacity={0.15} />
                ...
                <Tooltip
                  content={
                    <TT
                      fmt={(v, n) =>
                        n === "Median Salary" ? fmtSal(v) : v.toLocaleString()
                      }
                    />
                  }
                />
                <Area
                  ...
                  strokeWidth={2}
                  fill="url(#gSal)"
                  dot={{ fill: "#3cf0ff", r: 3, strokeWidth: 0 }}
                  activeDot={{
                    r: 6,
                    fill: "#3cf0ff",
                    stroke: "#030303",
              ...
              >
                ...
                {topRoleNames.map((rn, i) => (
                  <Area
                    ...
                    dataKey={rn}
                    stroke={TREND_PALETTE[i % TREND_PALETTE.length]}
                    fill={TREND_PALETTE[i % TREND_PALETTE.length]}
                    fillOpacity={0.06}
                    strokeWidth={2}
                    activeDot={{ r: 5, stroke: "#030303", strokeWidth: 2 }}
                  />
            ...
            <div className="space-y-2.5">
              ...
              {rising.length === 0 && (
                <p className="font-mono text-[11px] text-faint">
                  Run scraper multiple times to see growth trends
                </p>
              )}
            </div>
          </Card>
              ...
              {(data.meta?.sources_scraped ?? [])
                ...
                .map((s, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-mono text-[11px] text-tx">
                        {s.source} — {s.count} jobs
                      </div>
