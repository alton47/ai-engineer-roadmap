...
export default function Jobs({ data }: { data: AggregatedData }) {
  ...
  const roles = useMemo(
    () =>
      [...new Set((jobs ?? []).map((j) => j.role_type).filter(Boolean))].sort(),
    [jobs],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "title")
      list = [...list].sort(
        (a, b) => (a.title ?? "").localeCompare(b.title ?? "") * sortD,
      );
    if (sort === "salary")
      list = [...list].sort(
        (a, b) => ((a.salary_min_usd ?? 0) - (b.salary_min_usd ?? 0)) * sortD,
      );
    if (sort === "company")
      list = [...list].sort(
        (a, b) => (a.company ?? "").localeCompare(b.company ?? "") * sortD,
      );
    return list;
  ...
  const Th = ({ k, children }: { k: Sort; children: React.ReactNode }) => (
    ...
    >
      {children}
      {k && (
        <span className="ml-1 opacity-30 group-hover:opacity-100">
          {sort === k ? (sortD === 1 ? "↑" : "↓") : "⇅"}
        </span>
      )}
    </th>
    ...
    <PageWrap>
      ...
      >
        <div className="relative flex-shrink-0">
          ...
          <input
            className="bg-bg2 border border-line text-tx font-mono text-[12px] pl-7 pr-3 py-1.5 rounded-lg w-44 sm:w-56 outline-none focus:border-line2 transition-colors placeholder:text-faint"
            placeholder="search…"
            value={q}
            onChange={(e) => {
              setQ(e.target.value);
      ...
      <Section>
        <div className="space-y-2 mb-5">
          ...
          <Filters>
            ...
            <FSep />
            <FL>Level:</FL>
            {["all", "junior", "mid", "senior", "staff"].map((l) => (
              <FB
                key={l}
                active={lvF === l}
                onClick={() => {
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {paged.map((j, i) => (
                  <tr
                    key={i}
                    className="group hover:bg-bg2/60 transition-colors"
                  >
                    <td className="py-2.5 px-3 border-b border-line/40">
                      <div className="text-[13px] font-semibold text-hi group-hover:text-acid transition-colors leading-tight">
                        {j.title}
                      </div>
                      <div className="font-mono text-[10px] text-faint mt-0.5 max-w-52 truncate">
                    ...
                    <td className="py-2.5 px-3 border-b border-line/40 font-mono text-[11px] text-muted whitespace-nowrap">
                      {j.salary_min_usd
                        ? `$${Math.round(j.salary_min_usd / 1000)}k`
                        : j.salary_raw || "—"}
                    </td>
                    <td className="py-2.5 px-3 border-b border-line/40">
                      <LevelChip v={j.seniority} />
                    </td>
                    <td className="py-2.5 px-3 border-b border-line/40">
                      <Chip>{j.source}</Chip>
                    </td>
                ...
                {paged.length === 0 && (
                  <tr>
                    <td
                      colSpan={8}
                      className="text-center py-20 font-mono text-[12px] text-faint"
                    >
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-t border-line">
              ...
              <div className="flex gap-1 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="px-3 py-1 rounded-lg border border-line font-mono text-[11px] text-muted disabled:opacity-25 hover:border-line2 hover:text-tx transition-colors"
                >
                  ← prev
                </button>
