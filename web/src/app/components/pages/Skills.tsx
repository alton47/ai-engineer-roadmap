...
export default function Skills({ data }: { data: AggregatedData }) {
  ...
  const filt = useMemo(
    () => (cat === "all" ? all : all.filter((s) => s.cat === cat)),
    [all, cat],
  );
  const max = filt[0]?.count ?? 1;

  const cloud = useMemo(
    () => all.filter((s) => s.cat === "cloud").slice(0, 8),
    [all],
  );
  const llm = useMemo(
    () => all.filter((s) => s.cat === "llm").slice(0, 8),
    [all],
  );
  ...
  return (
    <PageWrap>
      <PageHeader
        title="Skills Map"
        sub={`${filt.length} skills · sized and ranked by frequency from real postings`}
      />
      <Section>
        <Filters>
          <FL>Category:</FL>
          {(Object.keys(CAT_LABELS) as Cat[]).map((c) => (
            <FB
              ...
              active={cat === c}
              onClick={() => setCat(c)}
              accent={c === "all" ? "#b8ff3c" : CAT_COLORS[c]}
            >
              {CAT_LABELS[c]}
            </FB>
          ))}
        ...
        <Card className="mb-4" p="p-5">
          <CardLabel right={`${filt.length} skills · hover for count`}>
            Skill Cloud
          </CardLabel>
          <AnimatePresence>
              ...
              {filt.map((s) => {
                ...
                return (
                  <motion.span
                    ...
                    whileHover={{ scale: 1.18, transition: { duration: 0.12 } }}
                    onHoverStart={() => setHov(s.name)}
