...
export default function Graph({ data }: { data: AggregatedData }) {
  ...
  useEffect(() => {
    ...
    const initThree = async () => {
      ...
      starG.setAttribute(
        "position",
        new THREE.Float32BufferAttribute(starV, 3),
      );
      scene.add(
        new THREE.Points(
          starG,
          new THREE.PointsMaterial({ color: 0x2a2a2a, size: 0.3 }),
        ),
      );
      scene.add(group);
        ...
        if (hits.length > 0) {
          ...
          if (idx >= 0)
            setTooltip({
              x: ex - rect.left,
              y: ey - rect.top - 36,
              text: `${nodes[idx].name} (${nodes[idx].count} jobs)`,
            });
      ...
      window.addEventListener("mousemove", (e) =>
        onMove(e.clientX, e.clientY, e.clientX, e.clientY),
      );
      canvas.addEventListener("wheel", (e) => {
        camera.position.z = Math.max(
          40,
          Math.min(200, camera.position.z + e.deltaY * 0.05),
        );
      });
      ...
      canvas.addEventListener(
        "touchmove",
        (e) => {
          ry += (e.touches[0].clientX - tpx) * 0.007;
          rx += (e.touches[0].clientY - tpy) * 0.007;
          tpx = e.touches[0].clientX;
          tpy = e.touches[0].clientY;
        },
        { passive: true },
  ...
  return (
    <PageWrap>
      <PageHeader
        title="3D Skill Constellation"
        sub="Node size = frequency · edges = co-occurrence · drag to rotate · scroll to zoom · hover for details"
      />
      <Section>
        <div className="checker rounded-xl overflow-hidden border border-line relative mb-4">
          ...
          {tooltip && (
            <div
              className="absolute pointer-events-none font-mono text-[11px] bg-bg2 border border-line2 rounded-lg px-3 py-1.5 text-hi shadow-2xl z-20"
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mb-4">
          {Object.entries(CAT_LABELS).map(([k, v]) => (
            <div key={k} className="flex items-center gap-2">
              <div
                ...
                style={{
                  background: CAT_COL_CSS[k],
                  boxShadow: `0 0 6px ${CAT_COL_CSS[k]}`,
                }}
              />
          ...
          <div className="ml-auto font-mono text-[10px] text-faint">
            {nodes.length} nodes ·{" "}
            {nodes.reduce((a, b) => a + b.count, 0).toLocaleString()} total
            occurrences
          </div>
        </div>
        ...
        <div className="bg-bg1 border border-line rounded-xl p-5">
          <div className="font-mono text-[10px] tracking-[.12em] uppercase text-faint mb-4">
            All Skill Nodes
          </div>
          <div className="flex flex-wrap gap-2">
            {nodes.map((n) => (
              <motion.span
                ...
                style={{
                  borderColor: `${CAT_COL_CSS[n.cat]}22`,
                  color: CAT_COL_CSS[n.cat],
                  background: `${CAT_COL_CSS[n.cat]}08`,
                }}
                title={`${n.count} jobs`}
              >
                {n.name}
