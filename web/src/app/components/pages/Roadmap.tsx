...
const PATHS: Record<BG, Step[]> = {
  beginner: [
    ...
    {
      phase: "Phase 2",
      title: "Your First LLM Integration",
      status: "done",
      accent: "#b8ff3c",
    ...
    {
      phase: "Phase 5",
      title: "Deploy & Ship Something Real",
      status: "locked",
      accent: "#ff6b4a",
    ...
    {
      phase: "Phase 6",
      title: "Evals — The Career Differentiator",
      status: "locked",
      accent: "#3dffa0",
    ...
    {
      phase: "Portfolio",
      title: "Two Projects, Deployed, Measured",
      status: "locked",
      accent: "#ffc93c",
  ...
  backend: [
    ...
    {
      phase: "Phase 2",
      title: "pgvector on Your Existing Stack",
      status: "active",
      accent: "#3cf0ff",
    ...
    {
      phase: "Portfolio",
      title: "Add AI to Something You've Already Built",
      status: "locked",
      accent: "#3dffa0",
  ...
  ds: [
    {
      phase: "Reframe",
      title: "Your Eval Skills Are Rare",
      status: "done",
      accent: "#b8ff3c",
    ...
    {
      phase: "Phase 1",
      title: "LLM APIs + When to Fine-tune",
      status: "done",
      accent: "#b8ff3c",
    ...
    {
      phase: "Phase 2",
      title: "RAG as Feature Engineering",
      status: "active",
      accent: "#3cf0ff",
    ...
    {
      phase: "Phase 3",
      title: "Enough Engineering to Ship",
      status: "next",
      accent: "#9d7cff",
  ...
  ml: [
    {
      phase: "Day 1",
      title: "Replace model.forward() with API Call",
      status: "done",
      accent: "#b8ff3c",
  ...
  frontend: [
    {
      phase: "Your Edge",
      title: "Full-Stack AI Is Genuinely Rare",
      status: "done",
      accent: "#b8ff3c",
    ...
    {
      phase: "Phase 2",
      title: "LLM Integration + Streaming UI",
      status: "active",
      accent: "#3cf0ff",
...
const BG_META: Record<BG, { label: string; timeframe: string; pitch: string }> =
  {
    beginner: {
      ...
      timeframe: "6–8 phases",
      pitch: "Start here with minimal coding background",
    },
    backend: {
      label: "Backend Developer",
      timeframe: "4–5 phases",
      pitch: "Your API knowledge is 80% of the work already",
    },
    ds: {
      label: "Data Scientist",
      timeframe: "4–5 phases",
      pitch: "Evaluation is your superpower — lead with it",
    },
    ml: {
      label: "ML Engineer",
      timeframe: "3–4 phases",
      pitch: "Smoothest path — replace model.forward() with API call",
    },
    frontend: {
      label: "Frontend Developer",
      timeframe: "4–5 phases",
      pitch: "Rare full-stack AI edge most engineers don't have",
    },
  };
  ...
  return (
    <PageWrap>
      <PageHeader
        ...
        sub="Evidence-based paths derived from 1,700+ real job postings — what's actually required, not what tutorials suggest"
      />
      <Section>
        <Filters>
          <FL>I am a:</FL>
          {(Object.keys(PATHS) as BG[]).map((b) => (
            <FB key={b} active={bg === b} onClick={() => setBg(b)}>
              {BG_META[b].label}
            </FB>
          ))}
        </Filters>
        ...
        <div className="mb-6 p-4 rounded-xl border border-line bg-bg2 flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <div className="text-[14px] font-semibold text-hi">
              {meta.label}
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(to bottom, #b8ff3c 0%, #383838 100%)",
              }}
            />
            ...
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  {/* Timeline dot */}
                  <div
                    className="absolute -left-[21px] top-[6px] w-3.5 h-3.5 rounded-full border-2"
                    style={{
                      borderColor: "#030303",
                      background: isLocked ? "#383838" : step.accent,
                      boxShadow: isLocked
                        ? "none"
                        : `0 0 10px ${step.accent}60`,
                    }}
                  />
                  ...
                  <div className="bg-bg1 border border-line rounded-xl p-5 hover:border-line2 transition-colors relative overflow-hidden">
                    ...
                    {!isLocked && (
                      <div
                        className="absolute -top-16 -right-16 w-40 h-40 rounded-full opacity-5 blur-3xl"
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2.5 relative">
                      <div>
                        <div
                          className="font-mono text-[10px] font-semibold tracking-[.12em] uppercase mb-0.5"
                          style={{ color: isLocked ? "#383838" : step.accent }}
                        >
                          {step.phase}
                        </div>
                        <div className="text-[14px] font-bold text-hi leading-snug">
                          {step.title}
                        </div>
                      </div>
                      <span
                        className={`font-mono text-[9px] px-2 py-0.5 rounded-full border flex-shrink-0 mt-0.5 ${sbadge.cls}`}
                      >
                        {sbadge.label}
                      </span>
                    </div>

                    <p className="text-[13px] text-muted leading-relaxed mb-3.5 relative">
                      {step.body}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 relative">
                      {step.tags.map((tag) => (
                        <span
                          key={tag}
                          className="font-mono text-[10px] px-2 py-0.5 rounded-md border border-line bg-bg3 text-faint"
                        >
                          {tag}
                        </span>
                      ))}
