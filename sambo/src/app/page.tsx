...
export type PageId = "overview"|"skills"|"salary"|"trends"|"jobs"|"graph"|"roadmap"|"sources";
...
export default function App() {
  ...
  useEffect(() => {
    fetch("/api/data")
      .then(r => r.json())
      .then((d: AggregatedData & { _demo?: boolean }) => { setData(d); setIsDemo(!!d._demo); })
      .catch(() => {
        const stats = computeStats(DEMO_JOBS);
        stats.trend = DEMO_TREND;
        setData({ meta: { total_jobs: DEMO_JOBS.length, generated_at: new Date().toISOString(), sources_scraped: [] }, stats, jobs: DEMO_JOBS });
        setIsDemo(true);
      })
      .finally(() => setLoading(false));
  }, []);

  const nav = (p: PageId) => { setPage(p); if (window.innerWidth < 1024) setSidebarOpen(false); };
  const ml  = sidebarOpen ? "var(--sidebar, 232px)" : "0";

  return (
    <div className="flex h-screen overflow-hidden bg-bg">
      <Sidebar
        active={page} onNav={nav}
        totalJobs={data?.meta.total_jobs ?? 0}
        isDemo={isDemo} open={sidebarOpen}
        onToggle={() => setSidebarOpen(v => !v)}
      />

      <main className="flex-1 overflow-y-auto overflow-x-hidden transition-[margin] duration-300"
        style={{ marginLeft: ml }}>

        {/* Mobile topbar */}
        <div className="lg:hidden sticky top-0 z-10 flex items-center gap-3 px-5 py-3.5 border-b border-line" style={{ background: "#030303cc", backdropFilter:"blur(16px)" }}>
          <button onClick={() => setSidebarOpen(v => !v)} className="text-muted hover:text-tx transition-colors" aria-label="Menu">
          ...
          <span className="font-mono text-[11px] text-muted tracking-wide">AI Engineer Roadmap</span>
        </div>
          ...
          <AnimatePresence mode="wait">
            {data && (
              <>
                ...
                {page === "sources"  && <Sources  key="so" data={data} isDemo={isDemo} />}
              </>
            )}
          </AnimatePresence>
        )}
